...
void setup(void) {
	...
	foodResistance = convertAnalogToResistance(foodAverage,
			FOOD_SERIESRESISTOR);
	foodDegC = calculateCFromResistance(foodResistance, FOOD_THERMISTORNOMINAL,
			FOOD_BCOEFFICIENT, TEMPERATURENOMINAL);
	foodDegF = convertCtoF(foodDegC);

	grillAverage = sampleTempData(PROBE_1_PIN, NUMSAMPLES);
	grillResistance = convertAnalogToResistance(grillAverage,
			GRILL_SERIESRESISTOR);
	grillDegC = calculateCFromResistance(grillResistance,
			GRILL_THERMISTORNOMINAL, GRILL_BCOEFFICIENT, TEMPERATURENOMINAL);
...
void loop(void) {
	...
	if (incomingByte == probe1) { // Read and return probe #1
		...
		if (debug == 1) {
			foodResistance = convertAnalogToResistance(foodAverage,
					FOOD_SERIESRESISTOR);
			foodDegC = calculateCFromResistance(foodResistance,
					FOOD_THERMISTORNOMINAL, FOOD_BCOEFFICIENT,
					TEMPERATURENOMINAL);
			foodDegF = convertCtoF(foodDegC);
		...
		if (debug == 1) {
			grillResistance = convertAnalogToResistance(grillAverage,
					GRILL_SERIESRESISTOR);
			grillDegC = calculateCFromResistance(grillResistance,
					GRILL_THERMISTORNOMINAL, GRILL_BCOEFFICIENT,
					TEMPERATURENOMINAL);
			grillDegF = convertCtoF(grillDegC);
